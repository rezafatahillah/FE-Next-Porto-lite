...
import { USER_EDIT_BREADCRUMB } from "../../enums";
import { UserQuery } from "../../hooks";
import { UserEditGeneralForm } from "../forms";
...
export function UserEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = UserQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={USER_EDIT_BREADCRUMB(id, current?.name)}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

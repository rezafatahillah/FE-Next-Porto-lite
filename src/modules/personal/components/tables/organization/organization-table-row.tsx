...
interface Props {
  row: IOrganizationEntity;
  selected?: boolean;
  onSelectRow?: VoidFunction;
  onDeleteRow: VoidFunction;
}

export function OrganizationTableRow(props: Props) {
  const { row, selected, onSelectRow, onDeleteRow } = props;

  const { id, name, type, year, position } = row;
      ...
      <Grid item xs={12} sm={6} md={4}>
        ...
        >
          <MenuList>
            <MenuItem
              onClick={() => {
                router.push(paths.backOffice.personal.organization.edit(id));
                popover.onClose();
              }}
            >

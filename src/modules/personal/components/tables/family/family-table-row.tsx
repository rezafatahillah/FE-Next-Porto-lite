...
export function FamilyTableRow(props: Props) {
  const { row, selected, onSelectRow, onDeleteRow } = props;

  const { id, name, status, birthDate, education, job } = row;
      ...
      <Grid item xs={12} sm={6} md={4}>
        ...
        >
          <MenuList>
            <MenuItem
              onClick={() => {
                router.push(paths.backOffice.personal.family.edit(id));
                popover.onClose();
              }}
            >

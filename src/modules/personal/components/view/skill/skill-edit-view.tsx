...
import { SKILL_EDIT_BREADCRUMB } from "../../../enums";
import { SkillQuery } from "../../../hooks";
import { SkillEditGeneralForm } from "../../forms";
...
export function SkillEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = SkillQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={SKILL_EDIT_BREADCRUMB(id, "test")}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

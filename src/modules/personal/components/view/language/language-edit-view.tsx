...
import { LANGUAGE_EDIT_BREADCRUMB } from "../../../enums";
import { LanguageQuery } from "../../../hooks";
import { LanguageEditGeneralForm } from "../../forms";
...
export function LanguageEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = LanguageQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={LANGUAGE_EDIT_BREADCRUMB(id, current?.name)}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

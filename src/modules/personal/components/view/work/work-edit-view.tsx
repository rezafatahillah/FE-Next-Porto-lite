...
import { WORK_EDIT_BREADCRUMB } from "../../../enums";
import { WorkQuery } from "../../../hooks";
import { WorkEditGeneralForm } from "../../forms";
...
export function WorkEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = WorkQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={WORK_EDIT_BREADCRUMB(id, "test")}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

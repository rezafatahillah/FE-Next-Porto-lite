...
import { DoctypeQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IDoctypeEntity) => void;
}

export function DoctypeSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    DoctypeQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, doctypeHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === doctypeHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">Doctypes</Typography>

...
interface Props {
  ...
  onSelect?: (value: IOrganizationEntity) => void;
}

export function OrganizationSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    OrganizationQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, organizationHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === organizationHeight;

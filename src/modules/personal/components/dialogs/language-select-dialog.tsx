...
interface Props {
  ...
  onSelect?: (value: ILanguageEntity) => void;
}
...
export function LanguageSelectDialog(props: Props) {
  ...
  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    LanguageQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, languageHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === languageHeight;
      ...
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          ...
          <Button
            ...
            href={paths.backOffice.personal.language.create}
            target="_blank"
            size="small"
            variant="text"

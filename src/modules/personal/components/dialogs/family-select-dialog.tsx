...
interface Props {
  ...
  onSelect?: (value: IFamilyEntity) => void;
}

export function FamilySelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    FamilyQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, familyHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === familyHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">Familys</Typography>

          <Button
            component={RouterLink}
            href={paths.backOffice.personal.family.create}
            target="_blank"
            size="small"
            variant="text"

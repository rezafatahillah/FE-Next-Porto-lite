...
interface Props {
  ...
  onSelect?: (value: IMedicalEntity) => void;
}

export function MedicalSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    MedicalQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, medicalHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === medicalHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">Medicals</Typography>

          <Button
            component={RouterLink}
            href={paths.backOffice.personal.medical.create}
            target="_blank"
            size="small"
            variant="text"

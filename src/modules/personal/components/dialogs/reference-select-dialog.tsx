...
interface Props {
  ...
  onSelect?: (value: IReferenceEntity) => void;
}

export function ReferenceSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    ReferenceQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, referenceHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === referenceHeight;
      ...
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          ...
          <Button
            ...
            href={paths.backOffice.personal.reference.create}
            target="_blank"
            size="small"
            variant="text"

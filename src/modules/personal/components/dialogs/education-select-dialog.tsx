...
import { EducationQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IEducationEntity) => void;
}

export function EducationSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    EducationQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, educationHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === educationHeight;
      ...
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          ...
          <Button
            ...
            href={paths.backOffice.personal.education.create}
            target="_blank"
            size="small"
            variant="text"

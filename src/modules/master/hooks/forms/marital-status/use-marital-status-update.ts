...
import { MaritalStatusUpdateSchema } from "../../../schemes";
...
export const useMaritalStatusUpdate = (props: Props) => {
  const { id, current, onSuccess } = props;

  const { activeFile, onUpload } = useStorageDirectUpload({
    endpoint: endpoints.maritalstatus.getDetails(id),
  });
    ...
    onUpload(
      { file: file },
      {
        onSuccess: async (file) => {
          toast.success("Successfully updated marital status!");
  ...
  const methods = useForm({
    mode: "onChange",
    resolver: yupResolver(MaritalStatusUpdateSchema),
    defaultValues: {
      name: "",
    },
  });
  ...
  const mutation = MaritalStatusQuery.useUpdate({});
    ...
    mutation.mutate(
      { id, payload: data },
      {
        onSuccess: (data) => {
          ...
          toast.success("Successfully updated maritalstatus!");
        },
        onError: (error) => {
          const { message, fields } = errorResponseMap(error, {
            message: "Failed update maritalstatus!",
          });

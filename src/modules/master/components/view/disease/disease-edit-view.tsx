...
import { DISEASE_EDIT_BREADCRUMB } from "../../../enums";
import { DiseaseQuery } from "../../../hooks";
import { DiseaseEditGeneralForm } from "../../forms";
...
export function DiseaseEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = DiseaseQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={DISEASE_EDIT_BREADCRUMB(id, current?.name)}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

...
import { JOBFIELD_EDIT_BREADCRUMB } from "../../../enums";
import { JobFieldQuery } from "../../../hooks";
import { JobFieldEditGeneralForm } from "../../forms";
...
export function JobFieldEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = JobFieldQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={JOBFIELD_EDIT_BREADCRUMB(id, current?.name)}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

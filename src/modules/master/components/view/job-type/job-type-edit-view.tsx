...
import { JOBTYPE_EDIT_BREADCRUMB } from "../../../enums";
import { JobTypeQuery } from "../../../hooks";
import { JobTypeEditGeneralForm } from "../../forms";
...
export function JobTypeEditView(props: Props) {
  const { id } = props;

  const tabs = useTabs("general");

  const { data, refetch } = JobTypeQuery.useGetDetails({
    props: {
      id,
    },
  });
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Edit"
        links={JOBTYPE_EDIT_BREADCRUMB(id, current?.name)}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      />

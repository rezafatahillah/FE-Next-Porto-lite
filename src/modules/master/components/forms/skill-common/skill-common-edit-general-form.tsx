...
import { useSkillCommonUpdate } from "../../../hooks";
...
export function SkillCommonEditGeneralForm(props: Props) {
  const { id, current, onSuccess } = props;

  const { activeFile, onUploading, methods, onSubmit, isLoading } = useSkillCommonUpdate({
    id,
    current,
    onSuccess,
  });
    ...
    <Card>
      ...
      <Stack spacing={3} sx={{ p: 3 }}>
        ...
        >
          ...
          <Field.Text name="slug" label="Name" required />
          <Field.Switch name="published" label="Published" />
        </Box>

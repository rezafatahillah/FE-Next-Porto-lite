...
interface Props {
  ...
  onSelect?: (value: IDegreeEntity) => void;
}

export function DegreeSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    DegreeQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, degreeHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === degreeHeight;

...
import { MaritalStatusQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IMaritalStatusEntity) => void;
}

export function MaritalStatusSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    MaritalStatusQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, maritalstatusHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === maritalstatusHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">MaritalStatuss</Typography>

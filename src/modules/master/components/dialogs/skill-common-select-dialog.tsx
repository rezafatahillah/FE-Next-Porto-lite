...
import { SkillCommonQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: ISkillCommonEntity) => void;
}

export function SkillCommonSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    SkillCommonQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, skillcommonHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === skillcommonHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">SkillCommons</Typography>

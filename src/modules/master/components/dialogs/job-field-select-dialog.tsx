...
import { JobFieldQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IJobFieldEntity) => void;
}

export function JobFieldSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    JobFieldQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, jobfieldHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === jobfieldHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">JobFields</Typography>

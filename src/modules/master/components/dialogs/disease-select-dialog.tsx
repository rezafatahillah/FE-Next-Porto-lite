...
interface Props {
  ...
  onSelect?: (value: IDiseaseEntity) => void;
}

export function DiseaseSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    DiseaseQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, diseaseHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === diseaseHeight;

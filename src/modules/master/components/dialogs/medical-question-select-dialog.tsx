...
import { MedicalQuestionQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IMedicalQuestionEntity) => void;
}

export function MedicalQuestionSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    MedicalQuestionQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, medicalquestionHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === medicalquestionHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">MedicalQuestions</Typography>

...
import { JobTypeQuery } from "../../hooks";
...
interface Props {
  ...
  onSelect?: (value: IJobTypeEntity) => void;
}

export function JobTypeSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    JobTypeQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, jobtypeHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === jobtypeHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">JobTypes</Typography>

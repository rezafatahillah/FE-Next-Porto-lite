...
interface Props {
  ...
  onSelect?: (value: ISkillLevelEntity) => void;
}

export function SkillLevelSelectDialog(props: Props) {
  const { open = false, onClose, onSelect } = props;

  const [search, debouncedSearch, setSearch] = useDebouncedState("");

  const { data, isFetching, fetchNextPage, hasNextPage, isFetchingNextPage } =
    SkillLevelQuery.useInfiniteGetAll({
      props: {
        params: {
          q: debouncedSearch,
  ...
  const renderList = (
    <Scrollbar
      onScrollCapture={(event: any) => {
        const { scrollTop, scrollHeight, skilllevelHeight } = event.target;
        const isScrolledBottom = scrollHeight - scrollTop === skilllevelHeight;
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={open} onClose={onClose}>
      <DialogTitle sx={{ pb: 0 }}>
        ...
        >
          <Typography variant="h6">SkillLevels</Typography>

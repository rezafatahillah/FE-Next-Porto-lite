...
import { JobFieldTableToolbar } from "./job-field-table-toolbar";
import { JobFieldTableRow } from "./job-field-table-row";
import { JobFieldQuery, useJobFieldFilter, useJobFieldDelete } from "../../../hooks";
...
export function JobFieldTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useJobFieldFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = JobFieldQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useJobFieldDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <JobFieldTableToolbar rowCount={rowCount} />

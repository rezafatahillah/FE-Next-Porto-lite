...
import { SkillLevelTableToolbar } from "./skill-level-table-toolbar";
import { SkillLevelTableRow } from "./skill-level-table-row";
import { SkillLevelQuery, useSkillLevelFilter, useSkillLevelDelete } from "../../../hooks";
...
export function SkillLevelTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useSkillLevelFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = SkillLevelQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useSkillLevelDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <SkillLevelTableToolbar rowCount={rowCount} />

...
interface Props {
  row: ISkillCommonEntity;
  selected?: boolean;
  onSelectRow?: VoidFunction;
  onDeleteRow: VoidFunction;
}

export function SkillCommonTableRow(props: Props) {
  const { row, selected, onSelectRow, onDeleteRow } = props;

  const { id, name, published, slug } = row;
  ...
  return (
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell>
          <Stack spacing={2} direction="row" alignItems="center"><ListItemText primary={published} /></Stack>
        </TableCell>

        <TableCell>
          <Stack spacing={2} direction="row" alignItems="center"><ListItemText primary={slug} /></Stack>
        </TableCell>
      ...
      >
        <MenuList>
          <MenuItem
            onClick={() => {
              router.push(paths.backOffice.master.skillcommon.edit(id));
              popover.onClose();
            }}
          >

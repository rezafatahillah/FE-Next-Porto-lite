...
import { JobTypeTableToolbar } from "./job-type-table-toolbar";
import { JobTypeTableRow } from "./job-type-table-row";
import { JobTypeQuery, useJobTypeFilter, useJobTypeDelete } from "../../../hooks";
...
export function JobTypeTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useJobTypeFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = JobTypeQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useJobTypeDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <JobTypeTableToolbar rowCount={rowCount} />

...
import { DegreeTableToolbar } from "./degree-table-toolbar";
import { DegreeTableRow } from "./degree-table-row";
import { DegreeQuery, useDegreeFilter, useDegreeDelete } from "../../../hooks";
...
export function DegreeTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useDegreeFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = DegreeQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useDegreeDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <DegreeTableToolbar rowCount={rowCount} />

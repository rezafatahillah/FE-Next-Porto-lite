...
import { ReligionTableToolbar } from "./religion-table-toolbar";
import { ReligionTableRow } from "./religion-table-row";
import { ReligionQuery, useReligionFilter, useReligionDelete } from "../../../hooks";
...
export function ReligionTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useReligionFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = ReligionQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useReligionDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <ReligionTableToolbar rowCount={rowCount} />

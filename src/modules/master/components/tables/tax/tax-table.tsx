...
import { TaxQuery, useTaxFilter, useTaxDelete } from "../../../hooks";
...
export function TaxTable(props: Props) {
  const {} = props;

  const confirm = useBoolean();

  const { queries } = useTaxFilter();

  const table = useTableParams();

  const { data, isFetching, refetch } = TaxQuery.useGetAll({
    props: {
      params: {
        ...queries,
        ...table.params,
  ...
  const { onSubmit: onDeleteRow } = useTaxDelete({
    onSuccess: () => {
      refetch();
    },
  });
  ...
  return (
    <>
      <TaxTableToolbar rowCount={rowCount} />
